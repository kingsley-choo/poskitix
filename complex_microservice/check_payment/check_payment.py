...
def check_payment(id):
    ...
    if r_queue_details.json()["data"]["status"]=="Ready":
    ...
    r_user = requests.get(f"http://user:5001/user/{r_queue_details.json()["data"]["uid"]}")
    if r_user.status_code // 100 != 2:
        return {"code":200, "data": "Success but notification not sent"}, 200
    
    r_event = requests.get(f"http://event/event/{r_queue_details.json()["data"]["eid"]}")
    if r_event.status_code // 100 != 2:
        return {"code":200, "data": "Success but notification not sent"}, 200 

    if r_queue_details.json()["data"]["status"] == "Done":
        r_createticket = requests.get(f"http://ticket:5003/ticket/event/{r_event.json()["data"]["eid"]}/user/{r_user.json()["uid"]}")
    print(r_createticket.text)
    exchange="email"
    body = {
        "user" : r_user.json(),
